...
import (
	...
	"github.com/windofbamboo/go-frame/go-oboci"
)
...
func Example_sqlSelect() {
	...
	if oci.TestDisableDatabase {
		fmt.Println(1)
		return
	}

	oci.Driver.Logger = log.New(os.Stderr, "oci ", log.Ldate|log.Ltime|log.LUTC|log.Lshortfile)

	var openString string
	// [username/[password]@]host[:port][/service_name][?param1=value1&...&paramN=valueN]
	if len(oci.TestUsername) > 0 {
		if len(oci.TestPassword) > 0 {
			openString = oci.TestUsername + "/" + oci.TestPassword + "@"
		} else {
			openString = oci.TestUsername + "@"
		}
	}
	openString += oci.TestHostValid

	// A normal simple Open to localhost would look like:
	// db, err := sql.Open("oci", "127.0.0.1")
	// For testing, need to use additional variables
	db, err := sql.Open("oci", openString)
	if err != nil {
...
func Example_sqlInsert() {
	...
	// For testing, check if database tests are disabled
	if oci.TestDisableDatabase || oci.TestDisableDestructive {
		fmt.Println(1)
		return
	}
	...
	// create table
	tableName := "E_INSERT_" + oci.TestTimeString
...
func Example_sqlManyInserts() {
	...
	// create table
	tableName := "E_MANY_INSERT_" + oci.TestTimeString
	query := "create table " + tableName + " ( A INTEGER )"
	ctx, cancel := context.WithTimeout(context.Background(), 55*time.Second)
	_, err = db.ExecContext(ctx, query)
	cancel()
...
func Example_sqlClob() {
	...
	// create table
	tableName := "E_CLOB_" + oci.TestTimeString
...
func Example_sqlBlob() {
	...
	// create table
	tableName := "E_BLOB_" + oci.TestTimeString
...
func Example_sqlRowid() {
	...
	// create table
	tableName := "E_ROWID_" + oci.TestTimeString
	query := "create table " + tableName + " ( A INTEGER )"
	ctx, cancel := context.WithTimeout(context.Background(), 55*time.Second)
	_, err = db.ExecContext(ctx, query)
	cancel()
