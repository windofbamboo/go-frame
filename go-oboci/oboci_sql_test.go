...
func testGetDB(params string) *sql.DB {
	Driver.Logger = log.New(os.Stderr, "oboci ", log.Ldate|log.Ltime|log.LUTC|log.Lshortfile)
	...
	db, err := sql.Open("oboci", openString)
	if err != nil {
...
func TestConnect(t *testing.T) {
	...
	// invalid
	db, err := sql.Open("oboci", TestHostInvalid)
	if err != nil {
	...
	// wrong username
	db, err = sql.Open("oboci", "dFQXYoApiU2YbquMQnfPyqxR2kAoeuWngDvtTpl3@"+TestHostValid)
	if err != nil {
		t.Fatal("open error:", err)
	}
	if db == nil {
